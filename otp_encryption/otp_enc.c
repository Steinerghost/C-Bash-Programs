...
int main(int argc, char* argv[]){
	...
	//Open the filename provided by the user for use in encryption
	...
	//Initalize a char array to send info to the encryption server
	char send_text[200000];

	//Recieve server name to confirm it's an encryption server
	memset(buffer, '\0', sizeof(buffer));
	...
	char input[100];
	strcpy(input, buffer);
	//If the server name is not "otp_enc_d"
	...
	//If the server did reply with "otp_enc_d", tag the sending information with true
	...
	//If the key size is less than the plaintext size, print an error
	if(strlen(keytext) < strlen(plaintext)){
		fprintf(stderr, "Key %s is too short.\n", argv[2]);
		exit(1);
	}

	//Compile the text being sent to the encryption server
	...
	//Prepare to recieve encoded message
	memset(buffer, '\0', sizeof(buffer));
